...
// Lazy config loader - only validates when actually accessed (at runtime)
function getConfig() {
  if (_configCache) {
    return _configCache;
  }
  ...
  if (!_logged) {
    console.log('[INFO] Loading server configuration at runtime');
...
// Use Proxy to intercept property access and lazy-load config
